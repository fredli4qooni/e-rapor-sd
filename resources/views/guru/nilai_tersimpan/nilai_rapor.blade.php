<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight uppercase">
            {{ __('Data Nilai Rapor Yang Terkirim') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 border-t-4 border-red-800">
                <div class="p-6">
                    <form method="GET" action="{{ route('guru.nilai-tersimpan.rapor') }}" class="space-y-4">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 border-t border-gray-200">
                        <thead class="bg-red-800 text-white">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider border-r border-red-900">Nama Siswa</th>
                                <th scope="col" class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider w-32 border-r border-red-900">NISN</th>
                                <th scope="col" class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider w-24 border-r border-red-900">NIS</th>
                                <th scope="col" class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider w-32 border-r border-red-900">Nilai Rapor</th>
                                <th scope="col" class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider w-32">
                                    @if($nilaiRapors->count() > 0)
                                        <form action="{{ route('guru.nilai-tersimpan.rapor.destroy', 'all') }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus SEMUA nilai rapor untuk kelas dan mata pelajaran ini? Data tidak dapat dikembalikan.');">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($nilaiRapors as $index => $nilai)
                                <tr class="hover:bg-red-50/50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-sm font-bold text-gray-700 border-r border-gray-200">{{ $nilai->siswa->nama_lengkap }}</td>
                                    <td class="px-4 py-3 text-center text-sm text-gray-700 border-r border-gray-200">{{ $nilai->siswa->nisn }}</td>
                                    <td class="px-4 py-3 text-center text-sm text-gray-700 border-r border-gray-200">{{ $nilai->siswa->nis }}</td>
                                    <td class="px-4 py-3 text-center text-sm text-gray-700 font-bold border-r border-gray-200">{{ $nilai->nilai_akhir }}</td>
                                    <td class="px-4 py-3 text-center text-sm">
                                        <form action="{{ route('guru.nilai-tersimpan.rapor.destroy', $nilai->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus nilai rapor untuk siswa ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="bg-red-600 hover:bg-red-700 text-white py-1 px-3 rounded shadow transition-colors text-xs font-semibold" title="Hapus">
                                                <svg class="w-3 h-3 inline-block mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                Hapus
                                            </button>
                                        </form>
