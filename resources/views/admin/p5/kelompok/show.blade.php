<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            {{ __('Detail Kelompok Kokurikuler') }}
        </h2>
    </x-slot>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-md shadow-md overflow-hidden">
                ...
                <div class="p-4 bg-gray-50 border-b space-y-4">
                    ...
                    <form action="{{ route('admin.p5.kelompok.anggota.store', $kelompok->id) }}" method="POST" class="flex gap-2">
                        @csrf
                            ...
                            @forelse($kelompok->siswas as $index => $siswa)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 text-center">
                                        <form action="{{ route('admin.p5.kelompok.anggota.destroy', [$kelompok->id, $siswa->id]) }}" method="POST" onsubmit="return confirm('Hapus siswa dari kelompok?');">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="bg-white rounded-md shadow-md overflow-hidden">
                ...
                <div class="p-4 bg-gray-50 border-b">
                    <form action="{{ route('admin.p5.kelompok.kegiatan.store', $kelompok->id) }}" method="POST" class="flex gap-2">
                        @csrf
                        <select name="p5_proyek_id" class="flex-1 border-gray-300 rounded-md shadow-sm focus:border-red-500 focus:ring-red-500 text-sm" required>
                            <option value="">-- Pilih Kegiatan (Fase {{ $kelompok->fase }}) --</option>
                            @foreach($semuaKegiatan as $kegiatan)
                                @if(!$kelompok->proyeks->contains('id', $kegiatan->id))
                                    <option value="{{ $kegiatan->id }}">{{ $kegiatan->nama_proyek }}</option>
                                @endif
                            @endforeach
                        </select>
                        ...
                        <tbody>
                            @forelse($kelompok->proyeks as $index => $proyek)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 text-center">
                                        <form action="{{ route('admin.p5.kelompok.kegiatan.destroy', [$kelompok->id, $proyek->id]) }}" method="POST" onsubmit="return confirm('Hapus kegiatan dari kelompok?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:text-red-800 font-medium text-xs">Hapus</button>
                                        </form>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-4 py-4 text-center text-gray-500">Belum ada kegiatan yang dipilih.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
