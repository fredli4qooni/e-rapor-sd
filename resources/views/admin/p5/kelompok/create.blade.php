<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            {{ __('Tambah Kelompok Kokurikuler') }}
        </h2>
    </x-slot>
    ...
    <div class="py-6 space-y-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-md shadow-md overflow-hidden max-w-2xl mx-auto">
            <div class="bg-[#8B1515] text-white px-6 py-4 font-bold tracking-wider uppercase text-sm border-b border-red-800 flex justify-between items-center">
                <span>Form Tambah Kelompok Kokurikuler</span>
                <a href="{{ route('admin.p5.kelompok.index') }}" class="text-sm font-semibold text-red-100 hover:text-white">Kembali</a>
            </div>
            
            <form action="{{ route('admin.p5.kelompok.store') }}" method="POST" class="p-6">
                @csrf

                <div class="space-y-4">
                    <div>
                        <x-input-label for="nama_kelompok" :value="__('Nama Kelompok')" class="text-gray-700 font-semibold" />
                        <x-text-input id="nama_kelompok" name="nama_kelompok" type="text" class="mt-1 block w-full" :value="old('nama_kelompok')" placeholder="Cth: Kelompok 1 - Kelas 1A" required autofocus />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="tingkat_pendidikan" name="tingkat_pendidikan" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm">
                                ...
                                <option value="1" {{ old('tingkat_pendidikan') == '1' ? 'selected' : '' }}>Tingkat 1</option>
                                <option value="2" {{ old('tingkat_pendidikan') == '2' ? 'selected' : '' }}>Tingkat 2</option>
                                <option value="3" {{ old('tingkat_pendidikan') == '3' ? 'selected' : '' }}>Tingkat 3</option>
                                <option value="4" {{ old('tingkat_pendidikan') == '4' ? 'selected' : '' }}>Tingkat 4</option>
                                <option value="5" {{ old('tingkat_pendidikan') == '5' ? 'selected' : '' }}>Tingkat 5</option>
                                <option value="6" {{ old('tingkat_pendidikan') == '6' ? 'selected' : '' }}>Tingkat 6</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="fase" name="fase" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                                ...
                                <option value="A" {{ old('fase') == 'A' ? 'selected' : '' }}>Fase A</option>
                                <option value="B" {{ old('fase') == 'B' ? 'selected' : '' }}>Fase B</option>
                                <option value="C" {{ old('fase') == 'C' ? 'selected' : '' }}>Fase C</option>
                            </select>
