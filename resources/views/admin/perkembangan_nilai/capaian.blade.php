<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            {{ __('Riwayat Capaian Nilai Siswa') }}
        </h2>
    </x-slot>
    ...
    <div class="py-6 space-y-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <!-- Info Siswa -->
        <div class="bg-white p-6 rounded-md shadow flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6 border-l-4 border-red-800">
                ...
                <table class="w-full text-sm text-left text-gray-700">
                    <thead class="text-xs text-gray-700 bg-gray-100 uppercase border-b">
                        <tr>
                            ...
                            <th class="px-4 py-3 border-r min-w-[200px]">Mata Pelajaran</th>
                            @foreach($semesters as $semName)
                                <th class="px-4 py-3 text-center border-r whitespace-nowrap">{{ $semName }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        @php $no = 1; @endphp
                        @forelse($capaianData as $mapel => $nilaiPerSemester)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                @foreach($semesters as $semId => $semName)
                                    <td class="px-4 py-3 text-center border-r text-lg font-bold {{ isset($nilaiPerSemester[$semId]) && $nilaiPerSemester[$semId] < 70 ? 'text-red-600' : 'text-gray-800' }}">
