    ...
    <div class="py-6 space-y-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-md shadow-md overflow-hidden">
            <div class="bg-[#8B1515] text-white px-6 py-4 font-bold tracking-wider uppercase text-sm border-b border-red-800 flex justify-between items-center">
                <span>Form Tambah Data Projek</span>
                <a href="{{ route('admin.referensi_p5.proyek.index', ['fase' => request('fase'), 'p5_tema_id' => request('p5_tema_id')]) }}" class="text-sm font-semibold text-red-100 hover:text-white">Kembali</a>
            </div>
            
            <form action="{{ route('admin.referensi_p5.proyek.store') }}" method="POST" class="p-6">
                @csrf

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <!-- Kolom Kiri: Informasi Proyek -->
                    <div class="space-y-4">
                        <h3 class="text-lg font-bold border-b pb-2 text-gray-800">1. Informasi Projek</h3>
                        
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <x-input-label for="no_urut" :value="__('Nomor Urut')" class="text-gray-700 font-semibold" />
                                <x-text-input id="no_urut" name="no_urut" type="number" class="mt-1 block w-full" :value="old('no_urut', 1)" required />
                            ...
                            <div>
                                ...
                                <select id="fase" name="fase" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required autofocus>
                                    ...
                                    <option value="A" {{ (old('fase') ?? request('fase')) == 'A' ? 'selected' : '' }}>Fase A (Kelas 1-2)</option>
                                    <option value="B" {{ (old('fase') ?? request('fase')) == 'B' ? 'selected' : '' }}>Fase B (Kelas 3-4)</option>
                                    <option value="C" {{ (old('fase') ?? request('fase')) == 'C' ? 'selected' : '' }}>Fase C (Kelas 5-6)</option>
                                </select>
                        ...
                        <div>
                            ...
                            <select id="p5_tema_id" name="p5_tema_id" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                                ...
                                @foreach($temas as $tema)
                                    <option value="{{ $tema->id }}" {{ (old('p5_tema_id') ?? request('p5_tema_id')) == $tema->id ? 'selected' : '' }}>
                                        {{ $tema->nama_tema }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <x-input-label for="nama_proyek" :value="__('Nama Data Projek')" class="text-gray-700 font-semibold" />
                            <x-text-input id="nama_proyek" name="nama_proyek" type="text" class="mt-1 block w-full" :value="old('nama_proyek')" placeholder="Cth: Cerdik Kelola Sampah Plastik" required />
                            <x-input-error class="mt-2" :messages="$errors->get('nama_proyek')" />
                        </div>

                        <div>
                        <x-input-label for="deskripsi" :value="__('Tujuan Akhir Projek')" class="text-gray-700 font-semibold" />
                        <textarea id="deskripsi" name="deskripsi" rows="3" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" placeholder="Contoh: Peserta didik mampu mengidentifikasi...">{{ old('deskripsi') }}</textarea>
                                                            ...
                                                            <div class="space-y-2">
                                                                @foreach($elemen->subElemens as $sub)
                                                                    <label class="flex items-start space-x-2 cursor-pointer hover:bg-gray-50 p-1 rounded">
                                                                        <input type="checkbox" name="sub_elemens[]" value="{{ $sub->id }}" class="mt-0.5 border-gray-300 rounded text-red-600 focus:ring-red-500" {{ in_array($sub->id, old('sub_elemens', [])) ? 'checked' : '' }}>
