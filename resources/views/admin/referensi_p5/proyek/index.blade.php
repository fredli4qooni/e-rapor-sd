<x-app-layout>
    ...
    <div class="py-6 space-y-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white p-4 rounded-md shadow flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <form action="{{ route('admin.referensi_p5.proyek.index') }}" method="GET" class="flex flex-col md:flex-row items-start md:items-center gap-4 w-full md:w-auto">
            ...
            <a href="{{ route('admin.referensi_p5.proyek.create', ['fase' => request('fase'), 'p5_tema_id' => request('p5_tema_id')]) }}" class="bg-red-600 hover:bg-red-500 text-white text-xs py-2 px-4 rounded shadow font-semibold shrink-0">
                + Tambah Projek
            </a>
        </div>
            ...
            <div class="p-6 overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-700 border">
                    ...
                    <tbody>
                        @forelse($proyeks as $proyek)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center flex gap-2 justify-center">
                                    <a href="{{ route('admin.referensi_p5.proyek.edit', $proyek->id) }}" class="text-blue-600 hover:text-blue-800 font-medium">Edit</a>
                                    <form action="{{ route('admin.referensi_p5.proyek.destroy', $proyek->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus projek ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-3 text-center text-gray-500">Belum ada data projek kokurikuler.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
