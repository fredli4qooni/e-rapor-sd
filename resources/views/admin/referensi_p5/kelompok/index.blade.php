<x-app-layout>
    ...
    <div class="py-6 space-y-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white p-4 rounded-md shadow flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <form action="{{ route('admin.referensi_p5.kelompok.index') }}" method="GET" class="flex flex-col md:flex-row items-start md:items-center gap-4 w-full md:w-auto">
            ...
            <a href="{{ route('admin.referensi_p5.kelompok.create') }}" class="bg-red-600 hover:bg-red-500 text-white text-xs py-2 px-4 rounded shadow font-semibold shrink-0">
                + Tambah Kelompok
            </a>
        </div>
                        ...
                        @forelse($kelompoks as $index => $kelompok)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center flex gap-2 justify-center flex-wrap">
                                    <a href="{{ route('admin.referensi_p5.kelompok.show', $kelompok->id) }}" class="text-green-600 hover:text-green-800 font-medium">Anggota/Projek</a>
                                    <a href="{{ route('admin.referensi_p5.kelompok.edit', $kelompok->id) }}" class="text-blue-600 hover:text-blue-800 font-medium">Edit</a>
                                    <form action="{{ route('admin.referensi_p5.kelompok.destroy', $kelompok->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kelompok ini?');" class="inline">
                                        @csrf
                                        @method('DELETE')
