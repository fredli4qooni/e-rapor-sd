    ...
    <div class="py-6 space-y-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-md shadow-md overflow-hidden">
            <div class="bg-[#8B1515] text-white px-6 py-4 font-bold tracking-wider uppercase text-sm border-b border-red-800 flex justify-between items-center">
                <span>Form Edit Mapping Pembelajaran</span>
                <a href="{{ route('admin.pembelajaran.index', ['rombel_id' => $pembelajaran->rombel_id]) }}" class="text-sm font-semibold text-red-100 hover:text-white">Kembali</a>
            </div>
            
            <form action="{{ route('admin.pembelajaran.update', $pembelajaran->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4 max-w-2xl">
                    <div>
                        ...
                        <select id="rombel_id" name="rombel_id" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                            ...
                            @foreach($rombels as $rombel)
                                <option value="{{ $rombel->id }}" {{ old('rombel_id', $pembelajaran->rombel_id) == $rombel->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="mata_pelajaran_id" name="mata_pelajaran_id" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                            ...
                            @foreach($mapels as $mapel)
                                <option value="{{ $mapel->id }}" {{ old('mata_pelajaran_id', $pembelajaran->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>
                                    {{ $mapel->nama_mapel }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="guru_id" name="guru_id" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                            ...
                            @foreach($gurus as $guru)
                                <option value="{{ $guru->id }}" {{ old('guru_id', $pembelajaran->guru_id) == $guru->id ? 'selected' : '' }}>
                                    {{ $guru->nama_lengkap }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="is_aktif" name="is_aktif" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                            <option value="1" {{ old('is_aktif', $pembelajaran->is_aktif) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('is_aktif', $pembelajaran->is_aktif) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
