            ...
            <form action="{{ route('admin.mapel.store') }}" method="POST" class="p-6">
                ...
                <div class="space-y-4 max-w-2xl">
                    ...
                    <div>
                        ...
                        <x-text-input id="nama_singkat" name="nama_singkat" type="text" class="mt-1 block w-full" :value="old('nama_singkat')" placeholder="Cth: PAI, PJOK" />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select id="is_transkrip" name="is_transkrip" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required>
                                <option value="1" {{ old('is_transkrip', '1') == '1' ? 'selected' : '' }}>Ya</option>
                                <option value="0" {{ old('is_transkrip') == '0' ? 'selected' : '' }}>Tidak</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="is_lokal" name="is_lokal" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm" required onchange="toggleMapelReferensi(this.value)">
                                <option value="0" {{ old('is_lokal', '0') == '0' ? 'selected' : '' }}>Bukan (Nasional)</option>
                                <option value="1" {{ old('is_lokal') == '1' ? 'selected' : '' }}>Ya (Muatan Lokal)</option>
                            </select>
                            <x-input-error class="mt-2" :messages="$errors->get('is_lokal')" />
                        </div>
                    </div>

                    <div id="div_mapel_referensi" class="{{ old('is_lokal') == '1' ? 'block' : 'hidden' }}">
                        ...
                        <select id="mapel_referensi_id" name="mapel_referensi_id" class="mt-1 block w-full border-gray-300 focus:border-red-500 focus:ring-red-500 rounded-md shadow-sm">
                            ...
                            @foreach($mapels as $m)
                                <option value="{{ $m->id }}" {{ old('mapel_referensi_id') == $m->id ? 'selected' : '' }}>
                                    {{ $m->nama_mapel }}
                                </option>
                            @endforeach
                        </select>
