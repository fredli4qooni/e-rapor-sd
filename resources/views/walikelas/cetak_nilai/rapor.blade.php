        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border-t-4 border-red-800">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-bold text-gray-800 mb-6 uppercase border-b pb-2">Cetak Nilai Rapor Siswa</h3>

                    <!-- Print Settings Form -->
                    <form method="GET" action="{{ route('walikelas.cetak_nilai.rapor.generate') }}" target="_blank" class="mb-8 p-4 border border-gray-200 rounded-md bg-white shadow-sm" id="print-settings-form">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-4">
                            ...
                            <div>
                                <label class="block font-semibold text-sm text-gray-700 mb-1">Halaman Pertama:</label>
                                <input type="number" name="halaman_pertama" value="{{ $setting->halaman_pertama ?? 1 }}" class="w-full border-gray-300 rounded-md text-center text-sm p-2 focus:ring-red-500 focus:border-red-500">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label class="block font-semibold text-sm text-gray-700 mb-1">Posisi Tanda Tangan KS:</label>
                                <select name="posisi_ttd_ks" class="w-full border-gray-300 rounded-md text-center text-sm p-2 focus:ring-red-500 focus:border-red-500">
                                    <option value="Sejajar Wali Kelas" {{ ($setting->posisi_ttd_ks ?? '') == 'Sejajar Wali Kelas' ? 'selected' : '' }}>Sejajar Wali Kelas</option>
                                    <option value="Bawah Wali Kelas" {{ ($setting->posisi_ttd_ks ?? '') == 'Bawah Wali Kelas' ? 'selected' : '' }}>Bawah Wali Kelas</option>
                                </select>
                            </div>
                            <div>
                                <label class="block font-semibold text-sm text-gray-700 mb-1">Tampil Nama Wali:</label>
                                <select name="tampil_nama_wali" class="w-full border-gray-300 rounded-md text-center text-sm p-2 focus:ring-red-500 focus:border-red-500">
                                    <option value="Isi Nama Wali Kelas" {{ ($setting->tampil_nama_wali ?? '') == 'Isi Nama Wali Kelas' ? 'selected' : '' }}>Isi Nama Wali Kelas</option>
                                    <option value="Tanpa Nama Wali Kelas" {{ ($setting->tampil_nama_wali ?? '') == 'Tanpa Nama Wali Kelas' ? 'selected' : '' }}>Tanpa Nama Wali Kelas</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-red-900 text-white shadow">
                                    <tr class="text-center text-xs uppercase">
                                        ...
                                        <th class="px-4 py-3 border border-red-800 w-48 align-middle">File Pdf Rapor Siswa<br>yang Tersedia</th>
                                        ...
                                        <th class="px-4 py-3 border border-red-800 w-48 align-middle">
                                            <div x-data="{ openAksi: false }" class="relative inline-block text-left">
                                                ...
                                                <div x-show="openAksi" style="display: none;" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 z-50">
                                                    <div class="py-1">
                                                        <form method="POST" action="{{ route('walikelas.cetak_nilai.rapor.toggle') }}">
                                                            @csrf
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-red-50 transition-colors {{ $index % 2 == 0 ? 'bg-red-50/20' : 'bg-white' }}">
                                        ...
                                        <td class="px-4 py-3 text-sm text-center border border-gray-300">
                                            <a href="{{ route('walikelas.cetak_nilai.rapor.generate', $siswa->id) }}" target="_blank" class="text-blue-500 hover:text-blue-700 hover:underline uppercase font-semibold text-xs">Nilai Rapor {{ $siswa->nama_lengkap }}</a>
                                        </td>
                                        <td class="px-4 py-3 text-sm text-center border border-gray-300">
                                            <a href="{{ route('walikelas.cetak_nilai.rapor.generate', $siswa->id) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-red-900 border border-transparent rounded-md font-bold text-xs text-white uppercase tracking-widest hover:bg-red-800 shadow-sm transition">
