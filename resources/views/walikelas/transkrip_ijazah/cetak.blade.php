<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight uppercase">
            {{ __('Cetak Transkrip Nilai Lampiran Ijazah') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border-t-4 border-red-800">
                <div class="p-6 text-gray-900">
                    <h3 class="text-lg font-bold text-gray-800 mb-6 uppercase border-b pb-2">Cetak Transkrip Nilai Lampiran Ijazah</h3>

                    <!-- Print Settings Form -->
                    <form method="GET" action="{{ route('walikelas.transkrip_ijazah.generate_pdf') }}" target="_blank" class="mb-8 p-4 border border-gray-200 rounded-md bg-white shadow-sm" id="print-settings-form">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            <div>
                                <label class="block font-semibold text-sm text-gray-700 mb-1">Jarak Antar Identitas :</label>
                                <input type="number" name="jarak_antar_identitas" value="{{ $setting->jarak_antar_identitas ?? 7 }}" class="w-full border-gray-300 rounded-md text-center text-sm p-2 focus:ring-red-500 focus:border-red-500">
                            </div>
                            <div>
                                <label class="block font-semibold text-sm text-gray-700 mb-1">Tinggi Judul Tabel Nilai :</label>
                                <input type="number" name="tinggi_judul" value="{{ $setting->tinggi_judul ?? 8 }}" class="w-full border-gray-300 rounded-md text-center text-sm p-2 focus:ring-red-500 focus:border-red-500">
                            </div>
                            <div>
                                <label class="block font-semibold text-sm text-gray-700 mb-1">Tinggi Isi Tabel Nilai :</label>
                                <input type="number" name="tinggi_isi_tabel" value="{{ $setting->tinggi_isi_tabel ?? 6 }}" class="w-full border-gray-300 rounded-md text-center text-sm p-2 focus:ring-red-500 focus:border-red-500">
                            </div>
                            <div>
                                <label class="block font-semibold text-sm text-gray-700 mb-1">Persentase Kop (%) :</label>
                                <input type="number" name="persentase_kop" value="{{ $setting->persentase_kop ?? 100 }}" class="w-full border-gray-300 rounded-md text-center text-sm p-2 focus:ring-red-500 focus:border-red-500">
                            </div>
                            <div>
                    ...
                    <div class="border border-gray-300 rounded-md bg-white">
                        <div class="flex justify-end p-4 gap-2 bg-gray-50 border-b">
                            ...
                            <button onclick="document.getElementById('print-settings-form').submit();" class="inline-flex items-center px-4 py-2 bg-red-900 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-800 transition">
                                ...
                                Generate Transkrip Nilai Ijazah Kelas ini
                            </button>
                            <!-- Secondary button for direct print, visually same logic in this demo -->
                            <button onclick="document.getElementById('print-settings-form').submit();" class="inline-flex items-center px-4 py-2 bg-red-900 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-800 transition">
                                ...
                                Cetak Langsung Transkrip Nilai Ijazah
                            </button>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-red-900 text-white shadow">
                                    <tr class="text-center text-xs uppercase">
                                        ...
                                        <th class="px-4 py-3 border border-red-800 w-48 align-middle">File Transkrip Nilai</th>
                                        ...
                                        <th class="px-4 py-3 border border-red-800 w-48 align-middle">
                                            <div x-data="{ openAksi: false }" class="relative inline-block text-left">
                                                ...
                                                <div x-show="openAksi" style="display: none;" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 z-50">
                                                    <div class="py-1">
                                                        <form method="POST" action="{{ route('walikelas.transkrip_ijazah.toggle_publikasi') }}">
                                                            @csrf
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-red-50 transition-colors {{ $index % 2 == 0 ? 'bg-red-50/20' : 'bg-white' }}">
                                        ...
                                        <td class="px-4 py-3 text-sm text-center border border-gray-300">
                                            <a href="{{ route('walikelas.transkrip_ijazah.generate_pdf', $siswa->id) }}" target="_blank" class="text-blue-500 hover:text-blue-700 hover:underline uppercase font-semibold text-xs">TRANSKRIP NILAI {{ $siswa->nama_lengkap }}</a>
                                        </td>
                                        <td class="px-4 py-3 text-sm text-center border border-gray-300">
                                            <a href="{{ route('walikelas.transkrip_ijazah.generate_pdf', $siswa->id) }}" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-red-900 border border-transparent rounded-md font-bold text-xs text-white uppercase tracking-widest hover:bg-red-800 shadow-sm transition">
                                        ...
                                        <td class="px-4 py-3 text-sm text-center border border-gray-300">
                                            <form method="POST" action="{{ route('walikelas.transkrip_ijazah.toggle_publikasi') }}">
                                                ...
                                                @if($siswa->is_transkrip_published)
