    ...
    <div class="text-center title">PENCAPAIAN KOMPETENSI PESERTA DIDIK<br>(KURIKULUM 2013)</div>
    
    <p>Nama Siswa: {{ $siswa->nama_lengkap }} (NIS: {{ $siswa->nis }})</p>
    ...
    <table>
        ...
        <tr>
            ...
            <td><strong>Predikat: {{ $sikap->predikat_spiritual ?? '-' }}</strong><br>{{ $sikap->deskripsi_spiritual ?? '-' }}</td>
        </tr>
        <tr>
            <td>Sikap Sosial</td>
            <td><strong>Predikat: {{ $sikap->predikat_sosial ?? '-' }}</strong><br>{{ $sikap->deskripsi_sosial ?? '-' }}</td>
        </tr>
    </table>
    ...
    <table>
        <tr>
            ...
            <th colspan="2">Pengetahuan & Keterampilan</th>
        </tr>
        <tr>
            <th width="10%">Nilai Akhir</th>
            <th>Capaian / Predikat</th>
        </tr>
        @if(isset($nilaiRapors) && $nilaiRapors->count() > 0)
            ...
            @php
                ...
                if (!$descTertinggi || $descTertinggi === '-') {
                    ...
                    if ($t && $t !== '-') {
                        $descTertinggi = "Sangat baik dalam " . rtrim(trim($t), '.') . ".";
                    }
                }
                if (!$descTerendah || $descTerendah === '-') {
                    $r = $nilai->deskripsi_terendah;
                    if ($r && $r !== '-') {
                        $descTerendah = "Perlu bimbingan dalam " . rtrim(trim($r), '.') . ".";
                    }
                }
            @endphp
            <tr>
